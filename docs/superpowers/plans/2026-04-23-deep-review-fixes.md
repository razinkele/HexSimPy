...
> **STATUS: ✅ EXECUTED** — All tasks closed. Perf/security: Liland 2024 DO thresholds for *S. salar* (`estuary.py`), contiguous-centroids cache (`mesh.py`), preallocated `OutputLogger` buffers (`output.py`), `push_temperature` `alive_mask` gating (`agents.py`), Numba/NumPy parity tests, activity-by-temperature decision doc. AST sandbox hardening (Tasks 5/5b/6): `_RNG_ARG_MAX = 1_000_000` enforced at `accumulators.py:167-234`; `_PROTECTED_LANDSCAPE_KEYS` at `events_phase3.py:78`. Task 1 (starvation physics) RESOLVED 2026-04-29 — Option A from `docs/superpowers/specs/2026-04-23-bioenergetics-starvation-decision.md` implemented: `BioParams.ED_TISSUE` bumped 5.0 → 36.0 (lipid-first); `update_energy()` rewritten with `mass_lost = actual_loss_j / (ED_TISSUE * 1000)`. Two new tests (`test_energy_density_declines_under_starvation`, `test_starvation_triggers_mortality_when_energy_depleted`) lock in the corrected physics. Full suite green: 811 passed.
