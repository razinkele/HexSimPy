...
> **STATUS: 🟡 PARTIAL** — Almost everything shipped. Done: Liland 2024 DO thresholds for *S. salar* (`estuary.py`), contiguous-centroids cache (`mesh.py`), preallocated `OutputLogger` buffers (`output.py`), `push_temperature` `alive_mask` gating (`agents.py`), Numba/NumPy parity tests, activity-by-temperature decision doc. AST sandbox hardening (Tasks 5/5b/6) DID ship: `_RNG_ARG_MAX = 1_000_000` enforced at `accumulators.py:167-234`; `_PROTECTED_LANDSCAPE_KEYS` at `events_phase3.py:78` blocks landscape-key collisions. **Genuine open thread:** Task 1 (starvation physics) shipped only *partially*. The Baltic-specific constant `ED_TISSUE = 36.0` is set at `baltic_params.py:54` with a docstring claiming lipid-first catabolism — but the algorithm change in `update_energy()` (Option A from the spec) was **never implemented** (zero `lipid|catabolism` references in `bioenergetics.py`; legacy proportional-mass formula still at lines 64-85). The spec at `docs/superpowers/specs/2026-04-23-bioenergetics-starvation-decision.md` is still header-marked `Status: BLOCKED — awaiting sign-off`. Test `test_energy_density_declines_under_starvation` does not exist. **Resolve by either** (a) running the modeler conversation and implementing Option A in `update_energy()`, OR (b) marking the algorithm change indefinitely deferred and updating the spec doc to match.
