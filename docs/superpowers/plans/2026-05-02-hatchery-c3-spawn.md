...
> **STATUS: ✅ EXECUTED 2026-05-02** — All 4 tasks complete. salmon_ibm/baltic_params.py gains `pre_spawn_skip_prob` field + `__post_init__` range validation + `_apply_hatchery_overrides` extended with `SCALAR_OVERRIDE_FIELDS` set + `dataclasses.replace(..., **scalar_kwargs)` mechanism; salmon_ibm/events_builtin.py inserts Bernoulli skip filter in `ReproductionEvent.execute` (gated by `hatchery_dispatch is not None and pre_spawn_skip_prob > 0`); configs/baltic_salmon_species.yaml gains `pre_spawn_skip_prob: 0.0` at wild + `pre_spawn_skip_prob: 0.3` in `hatchery_overrides` + full provenance comment block citing Bouchard 2022 + Christie 2014 + Jonsson 2019. Full pytest suite: **847 passed + 1 flaky perf test** (`test_movement_metric.py::test_full_step_time_within_one_percent_of_baseline` passes in isolation; project-level Windows-load flakiness, not C3.1-introduced — same flaky test as C2 run). Effective count 848 = 842 baseline + 6 new C3.1 tests; 0 regressions. Spec at `docs/superpowers/specs/2026-05-02-hatchery-c3-spawn-design.md`. Closes the first sub-tier of C3 (third in the C1 → C2 → C3 sequence); next: C3.2 sea-age sampling.
