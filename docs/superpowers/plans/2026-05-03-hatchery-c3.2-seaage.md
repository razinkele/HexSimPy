...
**Status:** ✅ EXECUTED on 2026-05-03. Final test count: 873 passing (baseline 848 + 25 new C3.2 tests in tests/test_hatchery_c3_2_seaage.py + 1 C1 backward-compat test fix). Branch `hatchery-c3.2-seaage` at HEAD `a204cdf`. Implementation commits: 586ae52 (Task 1) → aafd125+67a0b73 (Task 2) → a20eea2 (Task 3) → 0804198 (Task 4) → 97fe23e (Task 5) → 7c9f8ae (Task 6) → fe15f08 (Task 7) → f7a2dd2 (Task 8) → 4018706 (Task 9) → 1d01fea (Task 10) → a204cdf (Task 11 fix).
...
- Modify: `salmon_ibm/simulation.py:592-607` (step() landscape construction)
...
In `Simulation.step()` (around line 592-607), after the existing `"hatchery_dispatch": self.hatchery_dispatch,` line, add:
...
def test_simulation_step_injects_species_config_into_landscape():
    ...
    # with `"species_config": ...self._species_config...`. A regression
    # that drops this entry would crash CI here.
...
The inspect-source approach trades a small amount of robustness
against `Simulation.step`'s formatting for not needing the full
mesh/environment/scenario test scaffolding. If a future tier wants
end-to-end coverage, a heavier integration test can be added; the
contract this test locks (the field appears in both the TypedDict and
the construction site) is what the silent-failure-hunter flagged as
load-bearing.
