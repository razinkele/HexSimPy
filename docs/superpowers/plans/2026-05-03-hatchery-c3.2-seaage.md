...
def test_sea_age_override_path_rejects_numpy_int_keys():
    ...
    coercion. Without this test, a regression that drops the override-
    path type-check goes undetected because no test exercises numpy
    keys via the override entry point."""
    wild = BalticBioParams()
...
- [ ] **Step 3: Append a load-bearing test for the landscape injection**

This test is essential — without it, the wire-through is only validated
transitively via Tasks 6-8's tests, several of which hand-build a
synthetic landscape dict that bypasses `Simulation.step()` entirely.
A future refactor that breaks the `_species_config → landscape` flow
would silently emit all `-1`s in production while CI stays green.
...
def test_simulation_step_injects_species_config_into_landscape():
    ...
    mode in pass-1 review. This test locks the wire-through."""
    from salmon_ibm.events import Event, register_event

    captured: dict = {}

    @register_event("c3_2_landscape_capture")
    @dataclass
    class _CaptureLandscape(Event):
        def execute(self, population, landscape, t, mask):
            captured["species_config"] = landscape.get("species_config")

    # Minimal Simulation construction borrowing the existing Baltic
    # config. If the project has a `make_simulation`-style fixture in
    # tests/conftest.py or similar, prefer that; otherwise construct
    # by hand using the same arguments tests/test_simulation.py uses.
    # Accept either shape — what matters is that .step() runs and
    # the landscape dict reaches our spy event.
    from salmon_ibm.simulation import Simulation
    sim_cfg = {
        "scenario": {
            "events": [{"type": "c3_2_landscape_capture", "name": "spy"}],
            "n_steps": 1,
        },
        "species_yaml": str(CONFIG_PATH),
        # ... other minimal scaffolding the project's existing
        # test_simulation.py uses; reuse a fixture if present.
    }
    # If a from-config builder exists:
    # sim = Simulation.from_config(sim_cfg)
    # sim.step()
    # If only direct construction is supported, build the minimum and
    # call sim.step() — the test target is `captured["species_config"]
    # is sim._species_config`.
    sim = _build_baltic_test_simulation()  # define inline if no fixture
    sim.step()
    assert captured["species_config"] is sim._species_config
    assert captured["species_config"].wild.sea_age_distribution == \
        {1: 0.35, 2: 0.55, 3: 0.10}
```

If the codebase doesn't already have a Simulation test fixture, add
a minimal `_build_baltic_test_simulation()` helper at module scope
that constructs the smallest possible Simulation with a Baltic config
loaded. Pattern-match the existing `tests/test_simulation.py` to find
the right scaffolding (mesh, environment, scenario yaml). If the
project's Simulation construction is genuinely too heavy for this
test, fall back to a direct Landscape-construction unit test:

```python
def test_simulation_step_injects_species_config_into_landscape():
    """C3.2: load-bearing wire-through test (direct-construction
    variant if the full Simulation fixture is heavy)."""
    from salmon_ibm.simulation import Landscape  # TypedDict
    # Build the same dict literal that Simulation.step() builds.
    # The check is that the TypedDict declares species_config and that
    # the step() method (read inline) actually inserts it.
    import inspect
    src = inspect.getsource(Simulation.step)
    assert '"species_config"' in src and 'self._species_config' in src, (
        "Simulation.step() must inject species_config into landscape; "
        "C3.2 spec section 'simulation.py changes' mandates this entry."
    )
    # Verify TypedDict declares the field.
    assert "species_config" in Landscape.__annotations__
```

The inspect-source variant is admittedly fragile against refactors of
`Simulation.step`'s formatting, but it's a deterministic check that
the load-bearing wire-through is present without requiring full
Simulation construction. Pick whichever variant matches the
codebase's testing style; locking the invariant matters more than
the exact mechanism.

- [ ] **Step 4: Run test to verify pass**

Run: `micromamba run -n shiny python -m pytest tests/test_hatchery_c3_2_seaage.py::test_simulation_step_injects_species_config_into_landscape -v`
Expected: PASS.

- [ ] **Step 5: Run full suite to confirm no regression**

Run: `micromamba run -n shiny python -m pytest tests/ -x --ignore=tests/test_movement_metric.py -q`
Expected: pass count ≥ 848 + 14 (the new injection test brings the count from 13 to 14).

- [ ] **Step 6: Commit**

```bash
git add salmon_ibm/simulation.py tests/test_hatchery_c3_2_seaage.py
...
def test_introduction_event_n_zero_cohort():
    ...
    added, no rng.choice on empty `size=0`. Edge case the spec admits
    in the silent-failure analysis but the 13-test list does not
    explicitly cover for the IntroductionEvent code path."""
    pop, landscape = _baltic_landscape(seed=12345)
    evt = IntroductionEvent(
        name="empty_intro",
        n_agents=0,
...
def test_patch_introduction_n_zero_no_op():
    ...
    Closes the n=0 path through events_hexsim.py — without this test,
    a regression that calls `rng.choice(ages, size=0, p=probs)` could
    silently raise on empty `p` array."""
    pop = _make_population(n=0)
    cfg = load_baltic_species_config(CONFIG_PATH)
    landscape = {
        "rng": np.random.default_rng(12345),
...
Expected: AssertionError — without the sampling block, `pool.sea_age` stays at the default `SEA_AGE_UNSET = -1` for all introduced agents, so `np.all(np.isin(sea_ages, [1, 2, 3]))` returns False. (No NameError on `rng` because the test fixture sets `landscape["rng"]`; the unmodified production code simply doesn't sample.)
...
def test_switch_population_propagates_sea_age():
    ...
    (verified via grep). If the project later moves it, adjust the
    import; the API contract (register/get) is what matters."""
...
Expected: 848 + ~24 = ~872 passing. (The intermediate per-task expected
counts in earlier tasks are indicative; the precise total depends on
whether the load-bearing wire-through test in Task 5 picked the
direct-Simulation variant or the inspect-source variant. The
load-bearing principle: pass count grows monotonically; no test
introduced by C3.2 should regress an earlier task's count.) If
`tests/test_movement_metric.py::test_full_step_time_within_one_percent_of_baseline`
flakes (Windows-load-flake on this project), retry it in isolation:
