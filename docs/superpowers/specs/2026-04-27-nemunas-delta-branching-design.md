...
| 3 | Both landscapes exercise the agent-side hook; only the discharge NC gains the per-branch variable. (Realised during self-review pass 3: discharge is *not* in the landscape NC — it lives in `data/nemunas_discharge.nc`, loaded separately via `forcings.river_discharge.file`. Pass 7 also clarified that `configs/config_nemunas_h3.yaml` doesn't reference the discharge file at all — only `configs/config_curonian_baltic.yaml` does. So `Q_per_branch` is "data shape sitting unused on the test landscape" by design; the production scenario loads it once a future event consumes it.) | Production scenario ships the feature to the deployed app; test landscape keeps the fast (~30 s rebuild) integration loop. Agent-side fields (`natal_reach_id`, `exit_branch_id`) are exercised by both. |
